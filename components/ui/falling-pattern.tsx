...
type FallingPatternProps = React.ComponentProps<'div'> & {
  /** Primary color of the falling elements. Defaults to the design-token primary. */
  color?: string
  /** Background color of the pattern surface. Defaults to the design-token background. */
  backgroundColor?: string
  /** Animation duration in seconds. Lower = faster. Default 150. */
  duration?: number
  /** Blur intensity for the dotted overlay. Default '1em'. */
  blurIntensity?: string
  /** Dotted overlay density. Default 1. */
  density?: number
}

/**
 * Decorative animated backdrop for hero/landing surfaces.
 *
 * IMPORTANT: uses `var(--color-primary)` / `var(--color-background)` rather
 * than `var(--primary)` / `var(--background)` because the latter are bare
 * RGB triplets in our Tailwind v4 setup (see app/globals.css) and bare
 * triplets are not valid CSS colors on their own. The --color-* tokens
 * already wrap them in rgb().
 *
 * Respects prefers-reduced-motion: when the user prefers reduced motion,
 * the pattern renders statically without animating.
 */
      ...
      >
        <motion.div
          ...
          style={{
            ...
            ...(shouldReduceMotion ? { backgroundPosition: startPositions } : {}),
          }}
